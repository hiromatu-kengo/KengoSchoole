...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	int seCrash = LoadSoundMem("sound/cuash.mp3");
	...
	while (1) // メインループ
	{
		...
		for (int i = 0;i < COM_MAX;i++)
		{
			...
			{
				computerY[i] = computerY[i] - 1 - i;
				if (computerX[i] < -100)computerY[i] = HEIGHT + 100;
			}
		...
		if (scene == PLAY)//ゲーム中だけ出現
		{
			...
			if (abs(fuelX - playerX) < CAR_W[playerType] / 2 + 12 && abs(fuelY -
				playerY) < CAR_H[playerType] / 2 + 12)
			{
				fuelX = rand() % 180 + 270;
				fuelY = -500;
				fuel += 200;
		...
		drawText(WIDTH - 200, 10, 0x00ff00, "HI-SC %d", highScore, 30);
		int col = 0x00ff00;
