#include "DxLib.h"
#include "shootingGame_1.h" // ヘッダーファイルをインクルード
#include<stdlib.h>
...
enum { ENE_BULLET, ENE_ZAKO1, ENE_ZAKO2, ENE_ZAKO3, ENE_BOSS };//敵地の種類
...
int imgFighter, imgBullet;//時機と時機の玉の画像
...
int stage = 1;//ステージ
int score = 0;//スコア
int hisco = 10000;//ハイスコa
int noDamage = 0;//無敵状態
int weaponLv = 1;//自機の武器レベル(同時に発射される段数)
int scene = TITLE;//シーンを管理
int timer = 0;//アイテム用の構造体変数

struct OBJECT player;//時機用の構造体変数
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	initVariable();//【仮】ゲームを完成っセル際に呼び出し位置を変える
	...
	while (1) // メインループ
	{
		...
		case PLAY://ゲームプレイ画面
			...
			if (distance == STAGE_DISTANCE)
			{
				...
				PlaySoundMem(bgm, DX_PLAYTYPE_LOOP);//BGMループ出力
			}
			if (distance > 0)distance--;
			if (300 < distance && distance % 20 == 0)//雑魚1と2の出現
			{
			...
			if (300 < distance && distance < 900 && distance % 30 == 0)//雑魚3の出現
			{
				int x = 100 + rand() % (WIDTH - 200);
				int y = -50;
				int vy = 40 + rand() % 20;
			...
			if (distance == 1)bossIdx = setEnemy(WIDTH / 2, -120, 0, 1, ENE_BOSS, imgEnemy[ENE_BOSS], 200);//ボスの出現
		...
		case OVER:
			...
			{
				PlaySoundMem(jinOver, DX_PLAYTYPE_BACK);//神グル出力
			}
			else
			{
...
void initVariable(void)
{
	...
	for (int i = 0;i < ENEMY_MAX;i++)enemy[i].state = 0;//すべての敵機を存在しない状態に
	score = 0;
	stage = 1;
	noDamage = 0;
	weaponLv = 1;
...
void movePlayer(void)
{
	...
	static int countSpcKey;//スペースキーを押し続けている間カウントアップする変数
	...
	if (CheckHitKey(KEY_INPUT_SPACE))//スペースキー//スペースキー
	{
	...
	for (int n = 0;n < weaponLv;n++)
	{
		...
		for (int i = 0;i < BULLET_MAX;i++)
		{
			if (bullet[i].state == 0)//空いている配列に弾をセット
			{
				...
				bullet[i].vx = 0;
				bullet[i].vy = -40;//y軸方向の速さ(1回の計算で移動するピクセル数)
...
void moveBullet(void)
{
	for (int i = 0;i < BULLET_MAX;i++)
	{
		...
		bullet[i].x += bullet[i].vx;//座標を変化せせる
		bullet[i].y += bullet[i].vy;
		drawImage(imgBullet, bullet[i].x, bullet[i].y);//弾の描画
		if (bullet[i].y < -100)bullet[i].state = 0;//画面買いに出たら、存在しない状態にする
	}
}
//敵機をセットする
...
void moveEnemy(void)
{
	for (int i = 0;i < ENEMY_MAX;i++)
	{
		...
		if (enemy[i].pattern == ENE_ZAKO3)//雑魚機3
		{
			if (enemy[i].vy > 1)//減速
			{
				enemy[i].vy *= 0.9;
		...
		if (enemy[i].pattern == ENE_BOSS)//ボス機
		{
			...
			if (enemy[i].y < 120)
			{
				if (enemy[i].vy < 0)//弾発射
				{
					for (int bx = -2;bx <= 2;bx++)//二十ループのfor
						for (int by = 0;by <= 3;by++)
						{
							if (bx == 0 && by == 0)continue;
							setEnemy(enemy[i].x, enemy[i].y, bx * 2, by * 3,
								ENE_BULLET, imgEnemy[ENE_BULLET], 0);
						}
				}
				enemy[i].vy = 2;
			}
		}
		enemy[i].x += enemy[i].vx;//敵の移動
		enemy[i].y += enemy[i].vy;//
		drawImage(enemy[i].image, enemy[i].x, enemy[i].y);//敵機の描画
		//画面買いに出たか？
		if (enemy[i].x < -200 || WIDTH + 200 < enemy[i].x || enemy[i].y < -200
			|| HEIGHT + 200 < enemy[i].y)enemy[i].state = 0;
		//当たり判定のアルゴリズム
		if (enemy[i].shield > 0)//ヒットチェックを行う敵機（弾以外）
		{
			for (int j = 0;j < BULLET_MAX;j++)//自機の弾とヒットチェック
			{
				...
				int dx = abs((int)(enemy[i].x - bullet[j].x));//中心座標感覚のピクセル数
		...
		if (noDamage == 0)//無敵状態でないとき、自機とヒットチェック
		{
...
void stageMap(void)
{
	...
	DrawBox(mx, my, mx, mx + wi, my + he, 0xffffff, TRUE);
	...
	for (int i = 0;i < EFFECT_MAX;i++)
	{
		...
		switch (effect[i].pattern)//エフェクトごとに処理を分ける
		{
			...
			case EFF_RECOVER://回復演出
				...
				SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);//プ連弩モードを解除
				effect[i].timer++;
...
void moveItem(void)
{
	...
	item.timer++;
	DrawRectGraph(item.x - 20, item.y - 16, item.pattern * 40, 0, 40, 32, imgItem,
		TRUE, FALSE);
	//if(scene == OVER)return;//ゲームオーバー画面では回収できない
	int dx = item.x - player.x;
	int dy = item.y - player.y;
	int dis = dx * dx + dy * dy;
	if (dis < 60 * 60)//アイテムと時期とのヒットチェック(円による当たり判定)
	{
		item.state = 0;
